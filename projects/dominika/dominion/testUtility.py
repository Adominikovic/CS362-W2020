...
def get_random_box(box):
    boxlist = [k for k in box]
    random.shuffle(boxlist)
    random10 = boxlist[:10]
    return random10

def get_random_box1(box):
    boxlist = [k for k in box]
    random.shuffle(boxlist)
    random10 = boxlist[:5]
    return random10

def get_supply(box, boxlist):
    supply = defaultdict(list, [(k, box[k]) for k in boxlist])
    return supply


def get_supply_quantity(supply, player_names, nV, nC):
